...
c Convert to heliocentric coords and write a progress report to the log file
      if (abs(time-tlog).ge.abs(dtdump).and.opflag.ge.0) then
        ...
        tlog = time
c ##R##
        open (77, file='energies.txt', access='append')
		write(77,*) time/365.25
        close(77)
c ##R##
      end if
c
...
c
c ##R##
c      open(78, file='energies.txt', access='append')
c      close(78)
        open (77, file='energies.txt', access='append')
		write(77,*) pe, 0.5*ke, e 
c		write(77,*) 0.5*ke, v(1,1)*v(1,1)+v(2,1)*v(2,1)+v(3,1)*v(3,1), 
c     %                      v(1,2)*v(1,2)+v(2,2)*v(2,2)+v(3,2)*v(3,2)
        close(77)
c ##R##
c------------------------------------------------------------------------------
c
