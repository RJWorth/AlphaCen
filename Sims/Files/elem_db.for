...
c
      ...
      include 'mercury.inc'
c
      integer itmp,i,j,k,l,iback(NMAX),precision,lenin, ierr
...
c
c ##R## 
		open(77, file='velocities.txt', status='old')
c ##R## 
...
c Read in strings containing compressed data for each object
  323      do j = 1, nbod
           line_num = line_num + 1
           read (10,fin,err=666,iostat=ierr) c(j)(1:lenin)
		normal=' '
		if (c(j)(1:1).ne.normal) then
		write(*,'(a,i9,a)') "skipped",line_num,c(j)(1:10)
		goto 323
		endif
          end do
c
...
c
c Format statements
 213  format (1x,a8,1x,f12.4,1x,f10.5,1x,f7.3,1p,e11.4,0p,1x,f6.3,
     %  1x,f6.2)
c
      end
c
