...
c
c------------------------------------------------------------------------------
c Bug fix: Rachel Worth
c
c The criteria for determining whether a body has collided with the central
c object worked poorly for moons/satellites. I have added the requirement that
c in order for a collision to be scored, the body must have been able to reach
c the center at the velocity it was going, giving its location.
c
c The alteration is in the mce_cent subroutine and is marked with ##R##.
c
c------------------------------------------------------------------------------
c
c Some of Avi Mandell's changes are included here and are marked with 
c AVIMANDELL, including commenting out the output to the ce.out file because
c it's large and not useful to me, and commenting out writing output at every
c step, which causes it to run many times slower.
c His other modifications are omitted. (He also wrote in a runtime printout, 
c explicit file closing, and forced migration.)
c
c------------------------------------------------------------------------------
c Bug fix
...
c  **************   DGV Dimitri Veras added these parameters ************
      real*8 Omegag,rhog,delta
      parameter (Omegag=7.66963e-11,rhog=1.05629e-17,delta=-0.0573185)
c
c From Veras & Evans (2013) MNRAS
c
c R(kpc)      Omega         Delta          Rho
c  0.1    4.37096*10^-9    0.377521   9.06232*10^-15
c  0.2    2.75482*10^-9    0.283677   3.26618*10^-15
c  0.5    1.32134*10^-9    0.105695   6.48106*10^-16
c  1.0    6.78356*10^-10  -0.0214944  1.94041*10^-16
c  2.0    3.27146*10^-10  -0.0641641  8.42905*10^-17
c  3.0    2.13134*10^-10  -0.0469292  5.53658*10^-17
c  5.0    1.25414*10^-10  -0.0362197  2.77995*10^-17
c  8.0    7.66963*10^-11  -0.0573185  1.05629*10^-17
c 12.0    4.99957*10^-11  -0.0431656  3.12614*10^-18
c 20.0    2.99106*10^-11   0.0313698  4.43947*10^-19
c 35.0    1.74928*10^-11   0.0346311  8.98535*10^-20
c
c Local
      integer j
c  **************   ##R## Rachel Worth added these parameters ************
      real*8 aa(3,nbod),ab(3,nbod),cm(3)
      real*8 mtot,mgas0,mgas,rgas,gaslife,gasfrac
      character*80 bla
c
c  **************   DGV Dimitri Veras added this subroutine ************
c ##R## changed a to aa
c------------------------------------------------------------------------------
c
      do j = 2, nbod
        aa(1,j)= (Omegag**2.0)
     1         *((1.0-delta)*cos(2.0*Omegag*time) - delta)
     1         *x(1,j) +
     1          (Omegag**2.0)*(1.0-delta)*sin(2.0*Omegag*time)
     1         *x(2,j)

        aa(2,j)= (Omegag**2.0)*(1.0-delta)*sin(2.0*Omegag*time)*x(1,j) -
     1          (Omegag**2.0)*((1.0-delta)*cos(2.0*Omegag*time) + delta)
     1         *x(2,j)

        aa(3,j)= -(4.0*3.1415926535897932*K2*rhog - 
     1           2.0*(Omegag**2.0)*delta)*x(3,j) 
      end do
c
c------------------------------------------------------------------------------
c  **************   DGV Dimitri Veras end ************
c
c  **************   ##R## Rachel Worth added this subroutine ************
c------------------------------------------------------------------------------
c Initial mass of cloud, solar masses*K2
      mgas0 = 10*K2
c Lifetime of gas cloud (this and 'time' are in days)
      gaslife = 440000*365.25
c Mass over time, as cloud dissipates (...What about star accretion?)
      gasfrac= max(0.0, (gaslife-time)/gaslife)
      mgas = mgas0*gasfrac
c Radius of cloud core, in AU
      rgas = 7500
...
c Calculate location of center of mass
      do j = 2, nbod
...
c Calculate force from remnant gas cloud
      do j = 2, nbod
...
c Combine galactic tide and gas cloud effects
      do j = 2, nbod
        a(1,j)= aa(1,j) + ab(1,j)
...
c
c Input/Output
      integer algor,nbod,nbig,stat(nbod),opt(8),opflag,ngflag,nfun
      integer*8 ndump
      integer lmem(NMESS)
...
c AVIMANDELL, test for 'confirmdumpfile'
      logical test
...
c Update the data dump files
c AVIMANDELL
c  Data dump is commented; unnecessary at every output. Mistake?
c        do j = 2, nbod
c          epoch(j) = time
c        enddo
c
c        call mio_dump (time,tstart,tstop,dtout,algor,h0,tol,jcen,rcen,
c     %    rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,
c     %    id,ngf,epoch,opt,opflag,dumpfile,mem,lmem)
c        tdump = time
      endif
c
...
c
c AVIMANDELL
c  Added voluntary data dump when file 'confirmdumpfile' exists
c
...
c ##R## v0^2, needed for collision criterion modification
      real*8 vv0
c ##R##
c
c------------------------------------------------------------------------------
...
c ##R## v0^2, needed for collision criterion modification
          vv0 = vu0(1,j)*vu0(1,j) + vu0(2,j)*vu0(2,j) +vu0(3,j)*vu0(3,j)
c ##R##
        else
...
c ##R## v0^2, needed for collision criterion modification
          vv0 = v0(1,j)*v0(1,j) + v0(2,j)*v0(2,j) + v0(3,j)*v0(3,j)
c ##R##
        end if
c
...
c          if (q.le.rcen) then
c ##R## New collision criteria (must be within timestep*velocity of center)
          if ((q.le.rcen).and.((h*sqrt(vv0)).ge.(sqrt(rr0)))) then
c ##R##
            nhit = nhit + 1
            jhit(nhit) = j
            dhit(nhit) = rcen
...
c AVIMANDELL
c  *Commenting close encounter output for speed
c
...
c
c Input/Output
      integer algor,nbod,nbig,stat(nbod),opt(8),opflag,nfun
      integer*8 ndump
      integer lmem(NMESS)
...
c ##R##
c      bla='../../testing.txt'
c      open (77, file=bla, status='old', access='append')
c      write (77,*) " mio_dump dumpsteps = ",ndump
c      close(77)
c ##R##
c
...
c      bla='../../testing.txt'
c      open (77, file=bla, status='old', access='append')
c      write (77,*) "   mio_in dumpsteps = ",ndump
c      close(77)
c ##R##
      h0 = abs(h0)
      tol = abs(tol)
...
c ##R## 
c		if (k.eq.nbod) then
c			open(77, file='fv.txt', status='old',access='append')
c			write(77,*) c(k)(4+3*nchar:11+3*nchar), fv
c			close(77)
c		end if
c ##R## 
      end do
...
c ##R## 
		open(77, file='fv.txt', status='old',access='append')
		write(77,*) x,z
		close(77)
c ##R## 
c
